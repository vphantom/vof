...
| `matrix`          | `list[dims, sizes..., values...]                         | _same_             | Array (nested)                                               |
| `map`             | `list` of alternating keys and values                    | Map                | Object                                                       |
| `variant`/`enum`  | `list[Int,values…]` / `Int`                              | _same_             | Array[String,values…] / String                               |
| `struct`          | S.Open + groups… + S.Close                               | Map (keys: IDs)    | Object (keys: names)                                         |
| `series`          | Series + count + heads... + values... + Close            | Array (row 0: IDs) | Array (row 0: names)                                         |
...
Fixed-size multi-dimensional lists.  A single list/array starts with the number of dimensions, followed by the size of each dimension, then each value from zero-index onward, as many values as the product of all dimension sizes.

For example, a 3D array of 2x2x2 could be: `[3, 2, 2, 2, 1, 2, 3, 4, 5, 6, 7, 8]`

### Series

Compact representation of a list of `struct` where all the same fields are defined (typical of time series data, product price lists, etc.)  In CBOR/JSON this is a list of lists, where the first list specifies which fields are defined, as IDs in CBOR and names in JSON.  Each following list contains the values.  This is effectively a 2-D table where each row after the header is decoded into a `struct`.
